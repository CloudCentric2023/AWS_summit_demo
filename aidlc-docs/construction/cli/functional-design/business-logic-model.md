...
2. データが存在しない場合:
   a. 認証済み かつ オンラインの場合:
      i.  APIClient.Load() でリモートデータを取得
      ii. リモートにデータあり → ローカルに保存して使用（Step 3へ）
      iii. リモートにもデータなし → 新規ペット生成（初期値で作成）
   b. 未認証 or オフラインの場合:
      → 新規ペット生成（初期値で作成）
3. データが存在する場合:
...
**リモート復元の設計根拠:**
- 別端末で遊んでいたデータがクラウドに保存されている場合、新しい端末でも自然に復元される
- 認証済みユーザーの期待に沿う動作（ログインしているなら既存ペットが出てくる）
- リモート取得失敗時（ネットワークエラー等）は新規ペット生成にフォールバック
- 「手動セーブ」の設計とは矛盾しない（保存は手動、復元はInit時自動）
